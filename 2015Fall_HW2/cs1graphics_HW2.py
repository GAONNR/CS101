...
def checkColorName(colorName):
      """Check whether the colorName exists
      
      colorName   a string to check valid
      
      Return True if colorName is in Color._colorValues.
      Return False otherwise.
         
      """

      if colorName in Color._colorValues:
        return True
      else:
        return False
...
class Image(Drawable):
    ...
    if len(args) == 1:
      ...
      if filename[:7] != "base64:":
        ...
        if not _pilAvailable:
          ...
          if ext not in ['eps','gif','jpg','jpeg','png']:
            raise ValueError('Unsupported file type')
          _baseImage = _Image.open(filename).convert('RGB')
      else:
        data = _base64.b64decode(filename[7:])
        r = _cStringIO.StringIO(data)
        _baseImage = _Image.open(r).convert('RGB')
    elif len(args) == 2:
      if not isinstance(args[0], int) or not isinstance(args[1], int):
        raise TypeError('new image size must be integers')
      _baseImage = _Image.new('RGB', (args[0], args[1]), (255,255,255))
    else:
      raise ValueError('Image class only supports one or two arguments (%d given)' % (len(args)))
      
        
    self._size = _baseImage.size
    self._pixels = _baseImage.load()
    filename = self._make_ppm_string()
    
    _graphicsManager.loadImage(filename) # load it to see if its possible
    self._filename = filename
    self.force_to_reload = False
  ...
  def commit(self):
    ...
    if filename != None:
      self._filename = filename
      self.force_to_reload = True
      self._objectChanged(True,False,False)
  
  def _make_ppm_string(self):
    #ref: http://rosettacode.org/wiki/Bitmap/Write_a_PPM_file#Python     
    ret = 'P6\n'
    ret += '%d %d\n' % (self._size[0], self._size[1])
    ret += '255\n' # max value of RGB
    for y in range(self._size[1]):
          for x in range(self._size[0]):
            for c in self._pixels[x, y]:
              ret += chr(c)    
   
    return 'base64:' + _base64.b64encode(ret)         
